...
if pipe_or_person eq 0 then print, ' Pipeline  redshifts... '
if pipe_or_person eq 1 then print, ' Person VI redshifts... '
print
print

read, completeness_level, prompt=' Which sample:   8) for Stripe 82   9) For DR9Q.... ??'
;read, completeness_level, PROMPT=' - Whats the completeness level, 0.85 (DR9)  82 (S82) ?? '
if completeness_level eq 9 then completeness_level = 0.85
if completeness_level eq 8 then completeness_level = 82
print
print
...
if readin eq 'n' then begin
   ...
   ;; Normalized at z=2. 
;   kcorrection_file = 'kcorr_Miz2_mcgreer_1210.6389v1.dat'
;   kcorrection_file = 'kcorr_Miz2_mcgreer.dat'
;   kcorrection_file = 'kcorr_Miz2_mcgreer_R06splice.dat'
   kcorrection_file = 'kcorr_Miz2_mcgreer_R06spliced.dat'

;  readcol, 'Richards_2006_Table4.dat', kcor_redshift, kcor, /silent
   readcol, kcorrection_file, kcor_redshift, kcor;, /silent
   ;print, '  Richards06_kcor.dat READ-IN', n_elements(kcor)
   print
   print, 'The K-correction file is ', kcorrection_file, '  READ-IN', n_elements(kcor)
   print
   print
;   kcor_bin = kcor_redshift[1] - kcor_redshift[0]
...
;   qsomaskdir=filepath(root=getenv("BOSSQSOMASK_DIR"), "data/")
   ...
   if completeness_level eq 82 then begin
      ...
      s82_qso_zpconf_zW23 = where(data.ra ge 300.0 or data.ra lt 60.0 and $ 
                                  ...
                                  data.z_person ge 2.20 and data.z_person le top_zbin, $
                                  N_s82_qso_zpconf_zW23)
...
openw, 99, 'BOSS_QLF_statistical_temp.dat'
for ii=0ll, N_boss_qsos-1 do begin
   if completeness_level le 1.00 then begin
      printf, 99, boss[ii].ra, boss[ii].dec, $
              boss[ii].PSFMAG[3], boss[ii].EXTINCTION[3], $
              boss[ii].z, boss[ii].POLY_WEIGHT, $
              format='(d12.6,d12.6,  d12.6,d12.6,d12.6, d12.6,a2, d12.6  )'
   ENDIF ELSE BEGIN 
      printf, 99, boss[ii].ra, boss[ii].dec, $
              boss[ii].PSFMAG[3], boss[ii].EXTINCTION[3], $
              boss[ii].z, $
              format='(d12.6,d12.6,  d12.6,d12.6,d12.6, d12.6)'
   endELSE
endfor 
close, 99
...
;selec_func_file = 'fiducial_grid_i_sel.txt'
;selec_func_file = 'expdust_grid_i_sel.txt' 
...
print
print
print, 'USING THIS ::::      ', selec_func_file, '     :::: SELECTION FUNCTION!! '
print
...
if completeness_level le 1.00 then begin
   ...
   ;; openw, 32, 'My_QLF_iband_boss_formatted4paper_temp.dat'
   ;;   an unweighted version of the DR9 QLF is nice, but never really gonna
   ;;   get used/put in the paper...
   openw, 33, 'My_QLF_iband_boss_wgt_formatted4paper_temp.dat'
ENDIF ELSE BEGIN 
   openw, 12, 'My_QLF_iband_boss_S82_temp.dat'
   ;;openw, 13, 'My_QLF_iband_boss_S82_wgt_temp.dat'
   ;;  just doesn't make sense to weight-up the S82 data...
   openw, 32, 'My_QLF_iband_boss_S82_formatted4paper_temp.dat'
   ;; openw, 33, 'My_QLF_iband_boss_S82_wgt_formatted4paper_temp.dat'
   ;;   a Weighted version of the S82 QLF is nice, but never really gonna
   ;;   get used/put in the paper...
endelse
...
ENDIF ELSE BEGIN 
   openw, 14, 'My_QLF_iband_boss_narrowZ_S82_temp.dat'
;   openw, 15, 'My_QLF_iband_boss_narrowZ_S82_wgt_temp.dat'
   openw, 34, 'My_QLF_iband_boss_narrowZ_S82_formatted4paper_temp.dat'
 ;  openw, 35, 'My_QLF_iband_boss_narrowZ_S82_wgt_formatted4paper_temp.dat'
endelse
...
N_QSOs_boss_wgt = 0.0
log_Num_den_sum_boss     = 0.0
log_Num_den_sum_boss_wgt = 0.0
error_boss      = 0.0
error_boss_wgt  = 0.0
;;
;; WORKING OUT THE QLF
...
;      if N_QSOs_boss le 0 then printf, 12, z_bin, Abs_mag_bin, -9999.999, -999.,     N_QSOs_boss,     999.999
      if N_QSOs_boss gt 0 then begin
       ...
       ;  if N_selfn ne 0 then print, ii,jj, z_bin, Abs_mag_bin, Num_den_sum, log_Num_den_sum_boss, N_QSOs_boss, N_selfn, selfn[w_selfn]
         ...
         if completeness_level le 1.00 then begin
            ;;  *Can* have an Weighted DR9 calculation...
            ;;   though really doesn't make sense to have a S82
         ...
         if completeness_level ge 1.00 then begin
            printf, 32, mean_z, ' & ', mean_AbsMag, ' & ', Abs_mag_bin, ' & ', $
...
for ii = 0L, 10 do begin 
   ...
   for jj = 0L, 37 do begin 
      ...
      if N_QSOs_boss_wgt le 0 and completeness_level le 1.00 then $
         printf, 15, z_bin, Abs_mag_bin, -9999.999, -999.,     N_QSOs_boss_wgt, 999.999
