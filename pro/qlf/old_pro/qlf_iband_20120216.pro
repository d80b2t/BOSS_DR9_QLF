...
;   READ, completeness_level, PROMPT=' Which completeness level should be working to?? ' 
   print
   ...
   ENDIF ELSE BEGIN 
      w_boss = where(data.poly_weight ge completeness_level and $
                     data.z_conf_person ge 3 and data.z_person ge 2.00, N_boss_qsos)
   ENDELSE
...
;   boss = mrdfits('../../../bossqsomask/trunk/data/spall/mini-spAll-v5_4_45_Stripe82_z2QSOs.fits',1)
   ...
   if completeness_level eq 82 then boss = mrdfits(stripe82file, 1)
...
;
;readcol, '../../completeness/oldfiducial_grid_i_sel.txt', $
;readcol, '../../completeness/oldfiducial_grid_newlinesvrev_i_sel.txt', $
;readcol, '../../completeness/oldfiducial_grid_newlinesvrev_i_sel.txt', $
;readcol, '../../completeness/oldfiducial_grid_newlinesv5revx1d_i_sel.txt', $
