...
;      print, ii, jj, redmin, redmax, Abs_iM_min, Abs_iM_max, N_QSOs_dr3;, mean_z, V_comov, Num_den, log_Num_den
      ...
      if N_QSOs_dr3 gt 0 then begin
         ...
         Num_den = (float(N_QSOs_dr3)) / (V_comov * Area_dr3)
         ...
         ;; (1.0) Mag bin
         Num_den_per_mag     = Num_den     * (1.0 /mag_bins)
         Num_den_sum_per_mag = Num_den_sum * (1.0 /mag_bins)
         
         log_Num_den_dr3     = alog10(Num_den_per_mag)
         log_Num_den_sum_dr3 = alog10(Num_den_sum_per_mag)
         
         Abs_mag_bin = (Abs_iM_min + Abs_iM_max) /2.
         z_bin = (redmin + redmax) /2.
         printf, 11, z_bin, Abs_mag_bin, log_Num_den_dr3, log_Num_den_sum_dr3, N_QSOs_dr3
      endif
      ...
      if N_QSOs_boss gt 0 then begin
         ...
         V_comov = VCOMOVING(mean_z) / (1e6^3)
         Num_den = (float(N_QSOs_boss)) / (V_comov * Area_Stp82)
         
         Num_den_sum = 0.00000
         for kk = 0L, N_QSOs_boss-1 do begin
            ;print, ii, jj, kk, N_QSOs_boss
            z_Nth_QSO   = boss[w_qlfbin_boss[kk]].z
            V_comov_max =  VCOMOVING(z_Nth_QSO) / (1e6^3)
         ...
         log_Num_den_boss     = alog10(Num_den_per_mag)
         log_Num_den_sum_boss = alog10(Num_den_sum_per_mag)
         
         Abs_mag_bin = (Abs_iM_min + Abs_iM_max) /2.
         z_bin = (redmin + redmax) /2.
         printf, 12, z_bin, Abs_mag_bin, log_Num_den_boss, log_Num_den_sum_boss, N_QSOs_boss
      endif



      ;; FOR ZPHOT
      ;if N_QSOs_zphot gt 0 then begin
       ;  mean_z =  mean(zphot[w_qlfbin_zphot])
        ; 
         ;; Divide by (1e6^3) to get into Mpc 
         ;V_comov =  VCOMOVING(mean_z) / (1e6^3)
;         Num_den = (float(N_QSOs_zphot)) / (V_comov * Area_Stp82)
      
         ;; Alternatively you could implement the 1/Vmax here:
      ;   Num_den_sum = 0.00000
       ;  for kk = 0L, N_QSOs-1 do begin
        ;    z_Nth_QSO   = zphot[w_qlfbin_zphot[kk]]
         ;   V_comov_max =  VCOMOVING(z_Nth_QSO) / (1e6^3)
          ;  Num_den_sum = 1.00 / (V_comov_max * Area_Stp82) + Num_den_sum
         ;endfor
...
Dlum_min = 10^( (imin - Abs_iM_dr3 -  25.00) / 5.00) 
Dlum_max = 10^( (imax - Abs_iM_dr3 - 25.00) / 5.00) 
print, 'Dlum_min, Dlum_max calculated (DR3)'
print

;; D_comov also called D_M in Hogg99
D_comov_min = Dlum_min / (1.00 + z_dr3)
D_comov_max = Dlum_max / (1.00 + z_dr3)
print, 'D_comov_min, D_comov_max calculated (DR3)'
print


V_comov_min = ( (4d0 * !dpi)/3d0 * (D_comov_min^3)) * Area_dr3 
V_comov_max = ( (4d0 * !dpi)/3d0 * (D_comov_max^3)) * Area_dr3 
print, 'V_comovs done'
print
