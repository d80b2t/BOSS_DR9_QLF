...
;z_out       = (findgen(fltarr_bins*4.))/100.
;;
;; Okay, here we have 150 zbins, ranging from 
;; 0.000 to 5.960 in bins of 0.04
;;
...
openw, 10, 'chi_sq_PLE_model_a_temp.dat' 
...
phi_fit_d_out = fltarr(fltarr_bins,40)
openw, 14, 'chi_sq_PLE_model_d_temp.dat' 
for ii=0LL, n_elements(z_out)-1 do begin
      Mstar_g_out[ii]   = Mstar0_g_test  - 2.5*sqrt(k1_test*z_out[ii])
   for jj=0LL, n_elements(M_range_out)-1 do begin
      dmag_out      = M_range_out[jj] - Mstar_g_out[ii]
      ple_denom_test = (10.0^(0.4*(alpha_test+1)*(dmag_out))) + (10.0^(0.4*(beta_test+1)*(dmag_out))) 
      phi_fit_d_out[ii,jj] = Phi_star_test /ple_denom_test
      printf, 14, Z_out[ii], M_range_out[jj], phi_fit_d_out[ii,jj]
   endfor
endfor
close, 14
...
oplot, M_range,          alog10(phi_fit_a_full), thick=line_thick/2., color=purple,linestyle=0

oplot, MG_2SLAQ[w_plot], alog10(phi_fit_c[w_plot]), thick=line_thick, color=light_blue, linestyle=1 
oplot, M_range,          alog10(phi_fit_a_full), thick=line_thick/2., color=light_blue,linestyle=1

oplot, MG_2SLAQ[w_plot], alog10(phi_fit_c[w_plot]), thick=line_thick, color=orange, linestyle=1 
oplot, M_range,          alog10(phi_fit_a_full), thick=line_thick/2., color=orange,linestyle=1
...
oplot, MG_2SLAQ[w_plot], alog10(phi_fit_c[w_plot]), thick=line_thick, color=light_blue, linestyle=1 
oplot, M_range, alog10(phi_fit_a_full[600:749]), thick=line_thick/2., color=light_blue,linestyle=1
