...
plot, Abs_mag_bin_boss_s82[w] , log_Phi_BOSS_S82[w], $
      ...
      xstyle=1, ystyle=1, xthick=xthick, ythick=ythick, XTICKLEN=XTICKLEN, YTICKLEN=YTICKLEN, $
      charsize=charsize, charthick=charthick/1.4, thick=thick, $ 
...
;; Variables:
;;     alpha   is a power-law slope  
;;      beta   is a power-law slope  
;; 
;;
  ...
  ; alpha[alpha_i] = alpha_norm + (alpha_i*0.05)
   ...
   for beta_i = 0LL, beta_bins-1 do begin
      ...
      for phi_i = 0L, phi_star_bins-1 do begin
         ...
         ;Phi_star[phi_i] = 10^(Phi_star_norm+(phi_i*0.05))
...
;   mag_PLE[jj] = -32.0 + (jj*0.30)                       ;; to match the data's delta_mags
            ...
            Phi_PLE[jj, phi_i, alpha_i, beta_i] = Phi_star[phi_i]  / ( (10.0^(0.4*(alpha[alpha_i]+1)*(dmag))) + (10.0^(0.4*(beta[beta_i]+1)*(dmag))) )
