...
print
print, 'No. of QSOs included in the analysis, ', N_qsos_in, '  with minmax(zscan[qsos_in])  ', minmax(boss21_full[qsos_in].zscan)
print
print
...
mag_binsize      = 0.10 ;; the "standard", e.g. to match Richards06, is 0.25. 
                        ;; Here, it can also be 0.10 to match the McGreer selection function
...
;; Cumulative counts
no_count_bin_S82_sum  = fltarr(no_bins)

;for ii=0L, n_elements(field)-1 do begin
print, 'jj, bright_mag, faint_mag, mid_mag[jj], no_count_bin_S82[jj],  no_count_bin_S82_sum[jj]'
...
for jj=0L, no_bins-1 do begin
   ...
   bright_mag  = bright_mag_limit+(jj*mag_binsize)
   faint_mag   = bright_mag_limit+((jj+1)*mag_binsize)
   mid_mag[jj] = (bright_mag+faint_mag)/2.
   
   ;;
   ;; For  X D Q S O  -  D R 9  
   ;;
   w_dr9 = where(boss.poly_weight ge completeness_level and $
   ...
   printf, 10, mid_mag[jj], N_counts, N_counts/area_DR9, N_counts_sum, N_counts_sum/area_DR9
   
   
   ;;
   ;; For the 2.2 < z < 3.5 on the Stripe... 
   ;;
   w_s82 = where(boss_s82.z_person ge 2.20 and boss_s82.z_person lt 3.50 and $
   ...
   printf, 11, mid_mag[jj], N_counts_S82, N_counts_S82/area_S82, N_counts_S82_sum, N_counts_S82_sum/area_S82
   ...
   no_count_bin_S82_sum[jj] = N_counts_S82_sum
...
;                    N_counts_boss21)
   ...
   w_boss21 = where(boss21.zscan ge 1.00 and $
;                  boss21.z_conf_person ge 3 and $  ;; can take this or leave it... hmmmmm.... 
                    imag_boss21 lt faint_mag, $
                    N_counts_boss21_sum)

   print, jj, bright_mag, faint_mag, mid_mag[jj], N_counts_boss21, N_counts_boss21/area_boss21, N_counts_boss21_sum, N_counts_boss21_sum/area_boss21
   printf, 12, mid_mag[jj], N_counts_boss21, N_counts_boss21/area_boss21, N_counts_boss21_sum, N_counts_boss21_sum/area_boss21
