...
;;readnamed to:
;readcol, '../../pro/qlf/QLF_iband_sdss_narrowZ_S82_boss21_wSelfn_formatted_McGkcor.dat', $
;readcol, '../../pro/qlf/QLF_iband_sdss_S82_boss21_wSelfn_formatted_McGkcor_rv1.dat', $
readcol, '../../pro/qlf/QLF_iband_sdss_narrowZ_S82_boss21_wSelfn_formatted_McGkcor_rv1.dat', $
...
readcol, '../../pro/models/chi_sq_PLE_model_McG_0.3z2.2_rv1.dat', $
            ii_PLE, z_PLE, jj_PLE, Mi_PLE, k1_PLE, k2_PLE, alpha_PLE, Phi_PLE

w_PLE_range  = where(z_PLE ge 0.30 and  z_PLE le 2.05, N)
...
readcol, '../../pro/models/chi_sq_loglinear_LEDE_model_McG_2.2z3.5_rv1.dat', $
         ii_LEDE, z_LEDE, jj_LEDE, Mi_LEDE, c1_LEDE, c2_LEDE,  alpha, Phi_LEDE
...
;; y-ranges
ymin = -9.00   ;-10.0
ymax = -5.50  ;; was "traditionally" -5.50, e.g. w/o the LDDE model+offset
...
plot, z_full, log_phi_full, $
      psym=8, $
      position=[0.16, 0.12, 0.96, yposit_hi], $
      xrange=[xmin, xmax], $
      yrange=[ymin, ymax], $
...
if choice_plot_PLE eq 'y' then begin
   ...
   w_ple = where(Mi_PLE      lt binranges[8] and Mi_PLE      gt binranges[7], N_ple)
   ...
   w_ple = where(Mi_PLE      lt binranges[18] and Mi_PLE      gt binranges[17], N_ple)
...
if choice_plot_LEDE eq 'y' then begin
   ...
   w_lede = where(Mi_LEDE      lt binranges[8] and Mi_LEDE      gt binranges[7], N_lede)
   ...
   w_lede = where(Mi_LEDE      lt binranges[18] and Mi_LEDE      gt binranges[17], N_lede)
...
if N ge 5 then begin
   ...
   oploterror, z_full[w], log_Phi_full[w], z_full_err, up[w], $
               /hibar, errthick=errthick, psym=8, color=black+(clr_step*1), ERRcolor=black+(clr_step*1)
   oploterror, z_full[w], log_Phi_full[w], z_full_err, down[w], $
               /lobar, errthick=errthick, psym=8, color=black+(clr_step*1), ERRcolor=black+(clr_step*1)
...
;if N ge 5 then begin
   oplot, z_full[w], log_phi_full[w], color=black+(clr_step*14)+2, psym=8
   z_full_err = z_full[w] - z_full[w]
   oploterror, z_full[w], log_Phi_full[w], z_full_err, up[w], $
               /hibar, errthick=errthick, psym=8, color=black+(clr_step*14)+2, ERRcolor=black+(clr_step*14)+2
   oploterror, z_full[w], log_Phi_full[w], z_full_err, down[w], $
               /lobar, errthick=errthick, psym=8, color=black+(clr_step*14)+2, ERRcolor=black+(clr_step*14)+2
...
if plot_log_age eq 1 then xpos =  1.20 
...
xyouts, xpos_xy, -5.68, 'PLE!Dz=0.3-2.2!N',  charsize=charsize/1.4, charthick=charthick, color=black
xyouts, xpos_xy, -5.82, 'LEDE!Dz=2.2-3.5!N',  charsize=charsize/1.4, charthick=charthick, color=black

legend, '' , $
        position=[xpos_lab, -5.65], box=0, linestyle=1, color=black, $
