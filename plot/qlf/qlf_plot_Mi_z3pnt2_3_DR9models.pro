...
iMag_limit_line = fltarr(11, 101)
limit_lines = (findgen(101)/10.)-10.0                            
...
;; y-ranges
y_min = -10.00    ;; -9.20 
y_max = -4.50     ;; -4.70      ;; Used to be -5.00, so take care here!!
...
set_plot, 'ps'
!p.multi=0
device, filename='QLF_Mi_z3pnt2_3_DR9models_temp.ps', $
        xsize=8.0, ysize=8.0, $
...
oplot, iMag_limit_line[7,*]+0.1, limit_lines, color=boss_color, linestyle=1, thick=12


print
print, 'iMag_limit_line[7,0]  ', iMag_limit_line[7,0]  
print
...
;               M_BOSS lt iMag_limit_line[7,0] and log_Phi_boss gt -500, N) 
   www = where(z_bin_boss_wgt gt 3.00 and z_bin_boss_wgt lt 3.50 and $
               log_Phi_boss_wgt gt -50 and Mi_BOSS_wgt le -25.3, N) 
...
;   www = where(z_bin_boss gt 3.00 and z_bin_boss lt 3.50 and $
;               M_BOSS lt iMag_limit_line[7,0] and log_Phi_boss gt -500, N) 
   www = where(z_bin_boss_VdB gt 3.00 and z_bin_boss_VdB lt 3.50 and $
               log_Phi_boss_VdB gt -50 and Mi_BOSS_VdB le -25.3, N) 
...
;   www = where(z_bin_boss gt 3.00 and z_bin_boss lt 3.50 and $
;               M_BOSS lt iMag_limit_line[7,0] and log_Phi_boss gt -500, N) 
   www = where(z_bin_boss_Exp gt 3.00 and z_bin_boss_Exp lt 3.50 and $
               log_Phi_boss_Exp gt -50 and Mi_BOSS_Exp le -25.3, N) 
...
x_xyouts = -19.80    ;; to match the z~2.4 plot...
...
legend, '2SLAQ ' , $
        position=[x_xyouts+offset, y_xyouts-0.05+(0.5*offset)], $
        box=0, psym=8, color=color_2slaq, $
        charsiz=charsize_lab, charthick =charthick_lab

charsize_lab  = charsize/1.4
xyouts, x_xyouts-(0.8*offset), y_xyouts-0.05-(0.5*offset), $
        '!8(1.82<z<2.20)!3', $
...
charsize_lab  = charsize/1.1


loadct, clr_table
xyouts, x_xyouts+0.2, y_xyouts-0.05-(1.75*offset), '!3BOSS DR9', $
        charsize=2.6, charthick=8/1.2, color=black

plotsym, 0, 1.3, /fill
legend, 'fiducial' , $
        ;position=[xpos, ypos+(3*y_off)], $
        position=[x_xyouts+offset, y_xyouts-0.05-(2.005*offset)], $
        box=0, psym=8, color=boss_color, $
        charsiz=charsize_lab, charthick =charthick_lab

plotsym, plot_sym_exp, 1.3, /fill
legend, 'exp dust' , $
        position=[x_xyouts+offset, y_xyouts-0.05-(2.80*offset)], $
        box=0, psym=8, color=light_blue, $
        charsize=charsize_lab, charthick =charthick_lab

plotsym,  plot_sym_VdB, 1.3, /fill
legend, 'VdB lines' , $
        position=[x_xyouts+offset, y_xyouts-0.05-(3.60*offset)], $
        box=0, psym=8, color=color_dr9_VdB, $
        charsize=charsize_lab, charthick =charthick_lab

legend, ' ' , $
        position=[-18.90, -9.50], box=0, linestyle=1, color=boss_color, $
        charsize=1.0, charthick =12., thick= 16.0
;xyouts, -31.20, -9.0,'i!9A!321.8 limit', $
xyouts, -20.70, -9.70,'i=21.8 limit', $
        color=boss_color, charsize=charsize/1.2, charthick =charthick


;;
;;   z  ~  2 . 4 
;;
x_xyouts = -26.75 ;; -26.75
y_xyouts = -5.0   ;;  -5.50
offset   = 0.20

;xyouts, x_xyouts,    y_xyouts+(offset), '!8z~2.4!3', charsize=2.8, charthick=8, color=black
xyouts, x_xyouts+1.5, y_xyouts-(offset), '!83.00<z<3.50!3', charsize=2.6, charthick=8/1.2, color=black
