...
;;  B O S S   2 1
;;
readcol, '../../pro/qlf/My_QLF_iband_boss21_wSelfn_McGkcor.dat', $  ;; Used for the DR9 QLF paper..??
...
;   readcol, '../../pro/models/DeGraf12/qlf_z1pnt8to2pnt2.txt', $
   readcol, '../../pro/models/DeGraf12/qlf_z1pnt8to2pnt2_final.txt', $
...
set_plot, 'ps'
!p.multi=0
device, filename='QLF_Mi_z2pnt0_temp.ps', $
        xsize=8.0, ysize=8.0, $
...
;oplot, Mi_C17, log_phi_C17_npr, psym=8, color = orange
...
;if (choice_BOSS21_points eq 'y') then begin
   ...
   oplot, Mi_boss21[w21], log_Phi_BOSS21[w21], color=boss21_color, ps=8

   Mi_boss21_err = Mi_BOSS21[w21] - Mi_boss21[w21]
   oploterror, Mi_boss21[w21], log_Phi_BOSS21[w21], Mi_boss21_err, $
...
if (choice_fits eq 'y') then begin
   ...
   y_xyouts = -6.50     ;; to match the z~2.4 plot...
   x_off    =  0.50
   y_off    =  0.35     ;; 0.25
   offset   =  0.60
...
if (choice_models eq 'y') then begin
   x_xyouts = -19.30   ;; to match the z~2.4 plot...
   y_xyouts = -6.15     ;; to match the z~2.4 plot...
   x_off    = 0.50
   y_off    = 0.35     ;; 0.25
   offset = 0.60       ;; 0.40
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;;   M   O   D   E   L  S     AND    F I T S 
;;
;;
if choice_fits eq 'y' then begin
   loadct, clr_table
   
   ;w_PLE   = where(z_PLE_model_a   gt 1.98 and z_ple_model_a le 2.02, N_PLE)
;   oplot, MI2_PLE_MODEL_A[w_PLE], alog10(Phi_PLE_model_a[w_PLE]), $
 ;         thick=12, color=purple, linestyle = 0
   ...
   oplot, Mi_HRH07, alog10(phi_HRH07_z2pnt0)-per_mag, $
          thick=8, linestyle=2, color=turquiose 
   
;   loadct, 8
   w_PLE   = where(z_PLE   gt 2.35 and z_ple lt   2.45, N_PLE)
   oplot, mag_PLE[w_PLE], alog10(Phi_PLE[w_PLE]), $
          thick=12, color=purple, linestyle = 0

   loadct, clr_table
   w_LEDE = where(z_LEDE   gt 1.98 and z_LEDE  le 2.02, N_LEDE)
   oplot, mag_LEDE[w_LEDE], alog10(Phi_LEDE[w_LEDE]), $
          thick=12, color=red-8, linestyle = 3

   w_mLDDE = where(z_mLDDE gt 1.98 and z_mLDDE le 2.02, N_mLDDE)
...
if choice_models eq 'y' then begin
   ...
   ;; DeGraf et al. (2012)
   DeGraf12_err = abs(alog10(err_DeGraf12)-(log_phi_DeGraf12-per_mag))
   ;oplot,  Mi2_DeGraf12, log_phi_DeGraf12-per_mag, thick=thick, color=220, linestyle=0
   readcol, '../../pro/models/DeGraf12/qlf_z1pnt8to2pnt2_final.txt', $
            col1, col2, col3
   log_col3 = alog10(col3)
   ...
   ;Then you just need the standard graphics keywords (thick=* for the oplot
   ;line, and color=* for both).
;   polyfill, [Mi2_DeGraf12, reverse(Mi2_DeGraf12)], [(log_phi_DeGraf12-per_mag), reverse(log_col3-per_mag)], color=220
