...
;; Comes in raw M_g(z=2) no need to convert!!
...
Mg_BOSS_wgt = Abs_mag_bin_boss + Mg2_offset
...
M_boss21 = Abs_mag_bin_boss21 + Mg2_offset
...
!p.multi=0
device, filename='QLF_Mg_z2pnt4_temp.ps', $
        xsize=8.0, ysize=8.0, $
...
;               M_BOSS lt iMag_limit_line[7,0] and log_Phi_boss gt -500, N) 
   www = where(z_bin_boss_wgt gt 2.20 and z_bin_boss_wgt lt 2.60 and $
               log_Phi_boss_wgt gt -50 and Mg_BOSS_wgt le -24.0, N) 
   
   oplot, Mg_BOSS_wgt[www], log_Phi_boss_wgt[www], psym=8,   color=red
   ;oplot, M_gBOSS_wgt[www], log_Phi_boss_wgt[www], thick=12, color=red
...
;oplot,  Mg_SDSS[w], log_PhiR06[w], psym=8
