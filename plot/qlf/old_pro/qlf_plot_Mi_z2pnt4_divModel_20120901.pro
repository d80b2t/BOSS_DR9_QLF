...
print
print, '../../pro/qlf/My_QLF_iband_boss_wgt_20120619.dat  READ-IN', N_elements(z_bin_boss_wgt)
print
...
;; y-ranges
y_min =  -1.70
y_max =  1.40      ;; Used to be -5.00 for the z=2.6 plot so take care here!!
...
;               M_BOSS lt iMag_limit_line[7,0] and log_Phi_boss gt -500, N) 
   ...
   divmodel     = Hopkins_full_mag(2.00, Mi_BOSS_wgt[www])
   
   oplot,  Mi_BOSS_wgt[w],  alog10((10^log_Phi_boss_wgt[www]) /divmodel), $
           psym=8., color=black
   ...
   if (choice_DR9_errorbars eq 'y') then begin
      ...
      oploterror, Mi_SDSS[w], ratio, Mi_BOSS_err, diff_up,   $
                  /hibar, errthick=12, psym=8, color=boss_color, ERRcolor=boss_color
      oploterror, Mi_SDSS[w], ratio, Mi_BOSS_err, diff_down, $
...
if choice_how2plot eq '2' then oplot, Mi_C17,    alog10((10^log_Phi_C17) /divmodel), psym=8, color = clr_c17
