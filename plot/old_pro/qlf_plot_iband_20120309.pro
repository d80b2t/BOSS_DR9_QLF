...
;readcol, '../pro/qlf/My_QLF_iband_boss_S82_stnd.dat', z_bin_boss, Abs_mag_bin_boss, blah_boss,log_Num_den_boss, raw_N_QSOs_boss
...
;readcol, '../pro/qlf/My_QLF_iband_boss_S82_wExtinc.dat', $  ;; 
...
;readcol, '../pro/qlf/My_QLF_iband_boss21_wSelfn_temp.dat', $  ;; USE THIS FOR THE boss21 chunk!!
readcol, '../pro/qlf/My_QLF_iband_boss21_wSelfn_temp.dat', $  ;; USE THIS FOR THE boss21 chunk!!
...
if (choice_BOSS21_points eq 'y') then begin
   ...
   if (choice_BOSS21_npr eq 'y') then begin
 ;     oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
   ENDIF ELSE BEGIN 
;      oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
   ENDELSE
endif
...
if (choice_BOSS_points eq 'y') then begin
   ...
   oplot, Abs_Mag_bin_boss[www], log_Num_den_boss[www], color=boss_color, thick=12
endif
...
if (choice_BOSS21_points eq 'y') then begin 
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 0.68 and z_bin_boss21 lt 1.06 and RAW_N_QSOS_boss21 gt 0, N )
;   oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=boss_color, ps=8

   if (choice_BOSS21_npr eq 'y') then begin
 ;     oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], $
   ;          color=boss_color, ps=8
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 1.06 and z_bin_boss21 lt 1.44 and RAW_N_QSOS_boss21 gt 0, N )
   oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=boss_color, ps=8
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 1.44 and z_bin_boss21 lt 1.86 and RAW_N_QSOS_boss21 gt 0, N )
   oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 1.82 and z_bin_boss21 lt 2.20 and RAW_N_QSOS_boss21 gt 0, N) ; $
;                and Abs_Mag_bin_boss21 le iMag_limit_line, N )
   oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
...
www =  where(z_bin_boss gt 2.20 and z_bin_boss lt 2.60 and Abs_mag_bin_boss lt -24.5 and log_Phi_boss gt -500, N) 
if (choice_BOSS_points eq 'y') then begin
   loadct, 6
;   oplot, Abs_Mag_bin_boss[www], log_Num_den_boss[www], psym=8, color=boss_color
   oplot, Abs_Mag_bin_boss[www], log_Num_den_boss[www], color=boss_color, thick=12
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 2.20 and z_bin_boss21 lt 2.60 and RAW_N_QSOS_boss21 gt 0, N )
  ; oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
...
plotsym, 0, 1.5
plotsym, 0, plot_sym_size_BOSS, /fill
www =  where(z_bin_boss gt 2.60 and z_bin_boss lt 3.00 and Abs_mag_bin_boss lt iMag_limit_line[6,0] and log_Phi_boss gt -500, N) 
if (choice_BOSS_points eq 'y') then begin
   Mi_boss_err = log_Num_den_boss[www] - log_Num_den_boss[www]
   oploterror, Abs_Mag_bin_boss[www], log_Phi_boss[www], Mi_boss_err, boss_delta_up[www],  $
               /hibar, errthick=12, psym=8, color=boss_color, ERRcolor=boss_color
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 2.60 and z_bin_boss21 lt 3.00 and RAW_N_QSOS_boss21 gt 0, N )
   ;oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
endif
...
loadct, 6  ;; red  = 60 
plotsym, 0, plot_sym_size_BOSS, /fill
www =  where(z_bin_boss gt 3.00 and z_bin_boss lt 3.50 and Abs_mag_bin_boss lt iMag_limit_line[7,0] and log_Phi_boss gt -500,N) 
if (choice_BOSS_points eq 'y') then begin
   Mi_boss_err = log_Num_den_boss[www] - log_Num_den_boss[www]
   oploterror, Abs_Mag_bin_boss[www], log_Phi_boss[www], Mi_boss_err, boss_delta_up[www],  $
               /hibar, errthick=12, psym=8, color=boss_color, ERRcolor=boss_color
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 3.00 and z_bin_boss21 lt 3.50 and RAW_N_QSOS_boss21 gt 0, N )
   ;oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
endif
...
loadct, 6
plotsym, 0, plot_sym_size_BOSS, /fill
www = where(z_bin_boss gt 3.50 and z_bin_boss lt 4.00 and Abs_mag_bin_boss lt iMag_limit_line[8,0] and log_Phi_boss gt -500,N)  
if (choice_BOSS_points eq 'y') then begin
   Mi_boss_err = log_Num_den_boss[www] - log_Num_den_boss[www]
   oploterror, Abs_Mag_bin_boss[www], log_Phi_boss[www], Mi_boss_err, boss_delta_up[www],  $
               /hibar, errthick=12, psym=8, color=boss_color, ERRcolor=boss_color
   ...
   oplot, Abs_Mag_bin_boss[www], log_Num_den_boss[www], color=boss_color, thick=12
   ;; Giving a *very rough* feel for where a BOSS mag-limit would be...
   oplot, iMag_limit_line[8,*], limit_lines, color=boss_color, linestyle=1, thick=12
endif
...
if (choice_BOSS21_points eq 'y') then begin
   loadct, 6
   plotsym, 0, 1.5, /fill
   w21 =  where(z_bin_boss21 gt 3.50 and z_bin_boss21 lt 4.00 and RAW_N_QSOS_boss21 gt 0, N )
   ;oplot, Abs_Mag_bin_boss21[w21], log_Num_den_boss21[w21], color=80, ps=8
endif
...
loadct, 6
plotsym, 0, plot_sym_size_BOSS, /fill 
www =  where(z_bin_boss gt 4.00 and z_bin_boss lt 4.50 and Abs_mag_bin_boss lt iMag_limit_line[9,0] and log_Phi_boss gt -500,N) 
if (choice_BOSS_points eq 'y') then begin
   Mi_boss_err = log_Num_den_boss[www] - log_Num_den_boss[www]
   oploterror, Abs_Mag_bin_boss[www], log_Phi_boss[www], Mi_boss_err, boss_delta_up[www],  $
               /hibar, errthick=12, psym=8, color=boss_color, ERRcolor=boss_color
   ...
   oplot, Abs_Mag_bin_boss[www], log_Num_den_boss[www], color=boss_color, thick=12
   ;; Giving a *very rough* feel for where a BOSS mag-limit would be...
   oplot, iMag_limit_line[9,*], limit_lines, color=boss_color, linestyle=1, thick=12
endif

if choice_models eq 'y' then begin
   w_PLE   = where(z_PLE   gt 4.20 and z_ple lt   4.30, N_PLE)
   oplot, mag_PLE[w_PLE], alog10(Phi_PLE[w_PLE]), thick=8, linestyle = 0

   w_mLDDE = where(z_mLDDE gt 4.20 and z_mLDDE lt 4.30, N_mLDDE)
   oplot, mag_mLDDE[w_mLDDE], alog10(Phi_mLDDE[w_mLDDE]), thick=8, linestyle = 1

   w_LEDE = where(z_LEDE   gt 4.20 and z_LEDE  lt 4.30, N_LEDE)
   oplot, mag_LEDE[w_LEDE], alog10(Phi_LEDE[w_LEDE]), thick=8, linestyle = 2
endif

if (choice_models_HRH07 eq 'y') then begin
   readcol, '/cos_pc19a_npr/pdf/printed/Hopkins/HRH07_Bband_z4.25.dat', $
            band_lum_HRH07, a_HRH07, b_HRH07, bol_lum_HRH07, no_density_HRH07
...
plotsym, 0, plot_sym_size_BOSS, /fill
www =  where(z_bin_boss gt 4.50 and z_bin_boss lt 5.00 and Abs_mag_bin_boss lt iMag_limit_line[10,0] and log_Phi_boss gt -500, N)
if (choice_BOSS_points eq 'y') then begin
   Mi_boss_err = log_Num_den_boss[www] - log_Num_den_boss[www]
   oploterror, Abs_Mag_bin_boss[www], log_Phi_boss[www], Mi_boss_err, boss_delta_up[www],  $
               /hibar, errthick=12, psym=8, color=boss_color, ERRcolor=boss_color
   ...
   oplot, Abs_Mag_bin_boss[www], log_Num_den_boss[www], color=boss_color, thick=12
   ;;Giving a *very rough* feel for where a BOSS mag-limit would be...
   oplot, iMag_limit_line[10,*], limit_lines, color=boss_color, linestyle=1, thick=12
endif


if choice_models eq 'y' then begin
   w_PLE   = where(z_PLE   gt 4.70 and z_ple lt   4.80, N_PLE)
   oplot, mag_PLE[w_PLE], alog10(Phi_PLE[w_PLE]), thick=8, linestyle = 0

   w_mLDDE = where(z_mLDDE gt 4.70 and z_mLDDE lt 4.80, N_mLDDE)
   oplot, mag_mLDDE[w_mLDDE], alog10(Phi_mLDDE[w_mLDDE]), thick=8, linestyle = 1

   w_LEDE = where(z_LEDE   gt 4.70 and z_LEDE  lt 4.80, N_LEDE)
   oplot, mag_LEDE[w_LEDE], alog10(Phi_LEDE[w_LEDE]), thick=8, linestyle = 2
endif

if (choice_models_HRH07 eq 'y') then begin
   readcol, '/cos_pc19a_npr/pdf/printed/Hopkins/HRH07_Bband_z4.75.dat', $
            band_lum_HRH07, a_HRH07, b_HRH07, bol_lum_HRH07, no_density_HRH07
