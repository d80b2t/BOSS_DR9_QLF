...
;
; PURPOSE:
;   My "ultimate", all-singing, all-dancing, L-z plotting program...
;
; CALLING SEQUENCE:
;   > .run Lz
;
...
;    BOSS Quasar "catalogs"
;    2SLAQ QSO catalogs...
;
; OUTPUTS:
;   iband_redshift_temp.eps
;   Lz_temp.epse
;
; NOTES:
;
...
;                data.zscan ge 1.00,   N_QSOs_in)                                         
                data.zscan ge 0.68 and data.zscan le 2.20, N_QSOs_in)                                      
...
kcorrection_file = '../../pro/qlf/kcorr_Miz2_mcgreer_R06spliced.dat'
readcol, kcorrection_file, kcor_redshift, kcor_rv1 ;, /silent
print
print, 'The BOSS K-correction file is ', kcorrection_file, '  READ-IN', n_elements(kcor)
...
;; FOR SDSS DR7Q
;;
;; http://www.astro.princeton.edu/~schlegel/dust/local/local.html

glactc, dr7q.ra, dr7q.dec, 2000.0, dr7q_l, dr7q_b, 1, /DEGREE
eb_minus_v = dust_getval(dr7q_l, dr7q_b, /interp)
A_i = 2.086*eb_minus_v
dr7q_imag = dr7q.IMAG - A_i

;; FOR BOSS...
boss_imag   = boss.PSFMAG[3]   - boss.EXTINCTION[3]

;; FOR BOSS...
boss21_imag = boss21.PSFMAG[3] - boss21.EXTINCTION[3]
...
;   boss_Abs_iM = boss_imag - (5 * alog10(dlums_boss)) - 25.00 - kcor(fix(boss_z/0.01))
   boss_Abs_iM = boss_imag - (5 * alog10(dlums_boss)) - 25.00 - kcor_rv1(fix(boss.z/0.01)) 
endif
...
if choice_plot_ibandlimits eq 'y' then begin
   ...
   for ii=0ll, N_elements(i_band_mag)-1 do begin
      i_band_mag_bright[ii] = 17.80
      i_band_mag[ii]        = 21.80   ;;22.00  is 
   endfor
...
plot,  boss_imag,  boss_z, $
       ...
       color=black
;;
;; FOR THE i-band vs. redshift plots!!!
;;
plotsym, 0, 0.4, /fill
if choice_plot_sdss eq 'y' then begin
;   oplot, dr7q.IMAG, dr7q.z,  psym=8, color=black   ;; no extinction correction
   oplot, dr7q_imag, dr7q.z,  psym=8, color=black
   xyouts, 14.5, 4.8, 'SDSS DR7Q', charsize=charsize, charthick=charthick*1.6, color=black
endif

plotsym, 0, 0.33, /fill
if choice_plot_boss eq 'y' then begin
   oplot, boss_imag, boss_z,  psym=8, color=red
   xyouts, 14.5, 4.3, 'BOSS DR9', charsize=charsize, charthick=charthick*1.6, color=red
endif

plotsym, 8, 0.33
if choice_plot_boss21 eq 'y' then begin
;   oplot, boss21.psfmag[3], boss21.zscan,  psym=8, color=blue
   oplot, boss21_imag, boss21.zscan,  psym=8, color=blue
;   xyouts, 14.5, 3.8, 'BOSS S82 var', charsize=charsize, charthick=charthick*1.6, color=blue
   ;xyouts, 14.5, 3.8, 'boss21+MMT', charsize=charsize, charthick=charthick*1.6, color=blue
   xyouts, 14.5, 3.8, 'boss21', charsize=charsize, charthick=charthick*1.6, color=blue
endif
...
if choice_plot_ibandlimits eq 'y' then begin
   oplot, i_band_mag_red, Abs_i_limit_bright, linestyle=0, thick=8.0, color=turquiose
   oplot, i_band_mag_red, Abs_i_limit,        linestyle=0, thick=8.0, color=turquiose
endif
...
;; 3 samples:
xyouts, 2.50, -22.60, 'SDSS DR7Q',    charsize=charsize*1.2, charthick=charthick*1.8, color=black
xyouts, 2.50, -21.60, 'BOSS DR9 ',    charsize=charsize*1.2, charthick=charthick*1.8, color=red
;xyouts, 2.055, -20.70, 'BOSS S82 var', charsize=charsize, charthick=charthick*1.8, color=blue
xyouts, 2.5, -20.60, 'boss21+MMT', charsize=charsize*1.1, charthick=charthick*1.8, color=blue
;xyouts, 2.85, -20.70, 'boss21', charsize=charsize, charthick=charthick*1.8, color=blue

;xyouts, 2.7, -21.0, '2SLAQ QSO',  charsize=charsize*2.0, charthick=charthick*2.8, color=blue

if choice_plot_ibandlimits eq 'y' then begin
   xyouts, 4.3, -29.0, 'i=17.8',  charsize=charsize/1.2, charthick=charthick*1.4, color=turquiose
   xyouts, 4.3, -25.2, 'i=21.8',  charsize=charsize/1.2, charthick=charthick*1.4, color=turquiose
endif
...
;      plothist, boss_z,       bin=0.05, xhist, yhist, /noplot
      plothist, boss21.zscan, bin=0.05, /over, peak=max(yhist_bossz), thick =4.2*talk, color=blue, linesyle=2
   endif
   
   if choice_plot_sdss eq 'y' then begin
;      plothist,  dr7q_full.z, bin=0.05, /over, thick =4.2*talk, color=black, linestyle=1
      plothist, dr7q.z,  bin=0.05, xhist, yhist_dr7q, /noplot
      plothist,  dr7q.z, bin=0.05, /over, thick =4.2*talk, color=black, peak=max(yhist_bossz) 
   endif
...
xyouts, 0.06, 0.52,  $
        ...
        charthick=4.2, charsize=ycharsize*2.4
xyouts, 0.06, 0.57,  $
        '!N(!8z!3=2)', $
        ORIENTATION=90, /normal,$
        charthick=4.2, charsize=ycharsize*2.0
...
FOR j=0, numticks DO BEGIN
   XYOutS, xpos[j], ypos[j]-0.06, labels[j], $
           ...
           charsize=ycharsize*1.6, charthick=4.2
endfor
